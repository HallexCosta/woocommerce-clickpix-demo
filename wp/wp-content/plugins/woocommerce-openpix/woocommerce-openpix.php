...
function add_clickpix_script() {
  ...
  <script type="text/javascript">
      const injectOpenPixScriptAfterAllEvents = () => {
      ...
      const createClickPixButton = ({ productId, productPrice }) => {
          const clickpixButton = document.createElement("div");
          //  style
          clickpixButton.style.display = 'flex'
          clickpixButton.style.alignItems = 'center'
          clickpixButton.style.justifyContent = 'center'

          // attributes
          clickpixButton.setAttribute('disabled', 'true')

          // data
          clickpixButton.dataset.appid = "<?= WC_OpenPix_Pix_Gateway::instance()->getAppID() ?>";
          clickpixButton.dataset.clickpix = "";
          clickpixButton.dataset.variant = "purple";
          clickpixButton.dataset.customerFields = "phone,email,address";
          clickpixButton.dataset.additionalInfo = `product:${productId}`;
          clickpixButton.dataset.value = productPrice * 100;

          return clickpixButton
      }

      document.addEventListener("DOMContentLoaded", function() {
          for (const button of Array.from(document.querySelectorAll('.clickpix'))) {
              const productId = Number(button.dataset.productId)
              const productPrice = Number(button.dataset.productPrice)
              console.log(button)
              const clickPixButton = createClickPixButton({ productId, productPrice })
              console.log(clickPixButton)

              button.replaceWith(clickPixButton)

          }

          injectOpenPixScriptAfterAllEvents()
      })
  </script>
...
//        add_action( 'woocommerce_after_shop_loop_item', [$this, 'html_click_pix_button'], 10 );
//        add_action('wp_footer', 'add_clickpix_script');
    }
