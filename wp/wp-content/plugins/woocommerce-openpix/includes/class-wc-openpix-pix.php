...
// Função para adicionar um endpoint na REST API
function register_openpix_woocommerce_endpoints()
{
  remove_filter('rest_pre_serve_request', 'rest_send_cors_headers');
  header("Access-Control-Allow-Origin: *"); // Substitua '*' pelo domínio desejado, se necessário
  header("Access-Control-Allow-Methods: GET, POST, OPTIONS");
  header("Access-Control-Allow-Headers: Content-Type, Authorization");
  register_rest_route('openpix/v1/woocommerce', '/create-order-clickpix/', [
    'methods' => 'POST',
...
function createOrder(WP_REST_Request $request)
{
  ...
  try {
    ...
    $required_fields = ['taxID', 'value', 'correlationID', 'productId'];
    if (empty((array)$body)) {
      $response->set_data([
        'message' => 'Fields "' . implode(',', $required_fields) . '" is required'
      ]);
    ...
    foreach ($required_fields as $field) {
      if (!in_array($field, array_keys((array)$body))) {
        $not_informed_fields[] = $field;
      }
    }

    // inform the fields that was not informed
    if (!empty($not_informed_fields)) {
      $response->set_data([
        'message' => 'Field "' . implode(',', $not_informed_fields) . '" is required'
      ]);
      $response->set_status(400);
      return $response;
    }
    //  $price = isset($body['price']) ? sanitize_text_field($body['price']) : '';
    //  $email = isset($body['email']) ? sanitize_email($body['email']) : '';
    //  $phone = isset($body['phone']) ? wc_sanitize_phone_number($body['phone']) : '';

    // create order
    // $order = new WC_Order();
    $order = wc_create_order();
    $product = wc_get_product($body['productId']);
//    var_dump([$product, $body['productId']]);die;
    ...
    $order->set_payment_method('clickpix');
    $order->set_status('wc-pending');
    $meta_data = [
      'charge_correlation_id' => $body['correlationID'],
      'customer_tax_id' => $body['taxID'],
      'customer_email' => $body['email'],
      'customer_phone' => $body['phone'],
    ];
    if (!empty($body->userId)) {
      $meta_data['user_id'] = $body['userId'];
    }
...
add_action('rest_api_init', 'register_openpix_woocommerce_endpoints');

// inject openpix plugin script to active clicpix
function embedOpenPixPlugin()
{
  ?>
  <?php
  wp_enqueue_script(
  ...
  $scriptBeforeCreateCharge = <<<JS
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const createOrderWC = async (memoryOrder) => {
          ...
          try {
            console.log({body: {
              value: memoryOrder.payerPhone,
              email: memoryOrder.payerEmail,
              phone: memoryOrder.payerPhone,
              taxID: memoryOrder.payerTaxID,
              productId,
              correlationID: memoryOrder.correlationID,
              userId: 1
            }})
            console.log(`\${baseURL}/wp-json/openpix/v1/woocommerce/create-order-clickpix`)
            const response = await fetch(`\${baseURL}/wp-json/openpix/v1/woocommerce/create-order-clickpix`, {
              method: 'POST',
              headers: {
        ...
        window.\$openpixSDK.addEvent('beforeCreateCharge', async (nextFn, products, productId) => {
          ...
          window.\$openpixSDK.updateIn(productId, 'products', {
            ...
            additionalInfo: `orderId:\${orderId},\${oldProductOrder.additionalInfo}`,
            productId
          })
  ...
  wp_add_inline_script(
    'clickpix-react', // Run after of 'clickpix-react'
    $scriptBeforeCreateCharge
  );
}

add_action('wp_head', 'embedOpenPixPlugin');
...
class WC_OpenPix_Pix_Gateway extends WC_Payment_Gateway
{
  ...
  public function __construct()
  {
    ...
    // check if plugin is enabled and appID is not empty
    if ($this->get_option('enabled') === 'yes' && !empty($this->get_option('appID'))) {
      add_action('woocommerce_after_shop_loop_item', [
        $this,
        'html_click_pix_button'
      ]);
    }
  }

  public function html_click_pix_button()
  {
    global $product;
    $productId = $product->get_id();
    $productPrice = $product->get_price() * 100;
    $appID = $this->appID;
//    <div
//          data-appid="$appID"
//          data-variant="green"
...
//        </div>
    echo <<<HTML
       <div
          ...
          data-product-id="$productId"
          data-customer-fields="email,phone,address"
          data-additional-info="chargeType:clickpix,productId:$productId"
          data-value="$productPrice"
          class="btn-clickpix clickpix"
  ...
  public function isClickPixWebhookEvent($data)
  {
    $output = false;
    foreach ($data['charge']['additionalInfo'] as $info) {
      if ($info['key'] === 'chargeType' && $info['value'] === 'clickpix') {
        $output = true;
        break;
      }
    }
  ...
  public function handleWebhookEvents($data, $body)
  {
    ...
    if (
      ...
      if ($this->isClickPixWebhookEvent($data)) {
        $this->createClickPixOrder($data);
        return;
      }
  ...
  public function createClickPixOrder($data)
  {
    $status = $data['charge']['status'];
    ...
    $args = [
      ...
      'meta_value' => $data['charge']['correlationID'],
      'posts_per_page' => 1,
    ];
    ...
    if ($query->have_posts()) {
      header('HTTP/1.1 400 Bad Request');
      echo json_encode(['error' => 'correlation id already exists']);
    }

//    $userAlreadyCreated = get_user_by('email', 'hallex.costa@hotmail.com');
//
//    if (!$userAlreadyCreated) {
//      $customer = $data['charge']['customer'];
//      $userId = wp_insert_user([
//        'user_login' => $customer['email'],
//        'user_pass' => $customer['email'],
//        'display_name' => $customer['name'],
//      ]);
//
//      if ($userId instanceof WP_Error) {
//        header('HTTP/1.1 400 Bad Request');
//        echo json_encode(['error' => $userId->get_error_message()]);
//        return;
//      }
//    }

//    if (!in_array('admin', $userAlreadyCreated->roles) && !in_array('customer', $userAlreadyCreated->roles)) {
//      $this->addRoleCustomerToUser($userAlreadyCreated);
//    }

    foreach ($data['charge']['additionalInfo'] as $additionalInfo) {
        if ($additionalInfo['key'] === 'orderId') {
            $orderId = $additionalInfo['value'];
        }
    }
...
//    if ($userAlreadyCreated) {
//      $order->set_customer_id($userId ?? $userAlreadyCreated->ID);
//    }
//    $order->set_currency(get_woocommerce_currency());
//    $order->set_prices_include_tax('yes' === get_option('woocommerce_prices_include_tax'));
//    $order->set_customer_ip_address(WC_Geolocation::get_ip_address());
...
//    $order->set_customer_note('');
//    $order->set_payment_method('clickpix');
//    $order->set_transaction_id($data['pix']['transactionID']);
//    $order->set_total($data['charge']['value'] / 100);
    $order->set_status('wc-completed');
//    $order->set_meta_data([
//      'openpix_correlation_id' => $data['charge']['correlationID']
//    ]);
    $orderId = $order->save();
  ...
  public function addRoleCustomerToUser(WP_User $user)
  {
...
//  public function checkIfWCCustomerAlreadyExists($user_id)
//  {
//    global $wpdb;
